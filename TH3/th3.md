...
JavaScript đã được thực thi khi trang web được tải và hiển thị thông báo trong Console thông qua lệnh console.log().
---

#### Bước 2:  “JavaScript là gì?” (Tra cứu nhanh)
- JavaScript là một ngôn ngữ lập trình dùng để tạo sự tương tác và chức năng động cho trang web.
Sử dụng 1–2 nguồn tài liệu (vd. W3Schools, freeCodeCamp, …), tóm tắt:

> a) JavaScript chạy ở đâu? (Trình duyệt / Server / Cả hai?)
- JavaScript có thể chạy trong trình duyệt (browser) và trên server.
- Trong trình duyệt: dùng để tạo tương tác cho trang web.
- Trên server: chạy bằng môi trường như Node.js để xử lý dữ liệu, tạo API.
➡ Vì vậy JavaScript có thể chạy ở cả hai: Trình duyệt và Server.

> b) HTML, CSS, JavaScript mỗi phần chịu trách nhiệm chính về điều gì?
> - HTML: Dùng để tạo cấu trúc và nội dung của trang web (tiêu đề, đoạn văn, hình ảnh, form,...).
> - CSS: Dùng để thiết kế giao diện và định dạng trang web (màu sắc, font chữ, bố cục, responsive,...).
> - JavaScript: Dùng để tạo tương tác và xử lý logic trên trang web (click button, kiểm tra form, thay đổi nội dung, gọi API,...).
...
>   - `string` là: Kiểu dữ liệu chuỗi ký tự ( văn bản)................
>   - `boolean` là: Kiểu dữ liệu đúng hoặc sao(True/false)...........
...
> Câu hỏi:
> - Dòng log hiển thị gì sau khi em sửa thông tin? "Xin chào, mình là Tâm, năm nay mình 22 tuổi."
> - Nếu em quên dấu `;` hoặc quên dấu `+`, điều gì xảy ra? Trình duyệt báo lỗi thế nào?
Quên dấu ;
->Thông thường JavaScript vẫn chạy vì nó tự thêm dấu ;.
Nhưng trong một số trường hợp có thể gây lỗi logic.
Quên dấu +
->Trình duyệt sẽ báo lỗi trong Console như:
...
Console giúp chạy và kiểm tra code JavaScript nhanh chóng, xem kết quả và lỗi ngay lập tức khi lập trình.
> - Em gặp lỗi cú pháp nào? Em đã xử lý bằng cách nào (tự sửa, hỏi bạn, đọc lỗi, tìm Google, …)?
Em gặp lỗi sai cú pháp khi nối chuỗi (quên dấu + hoặc viết sai tên biến).
Em đã đọc thông báo lỗi trong Console và sửa lại code cho đúng cú pháp.
...
Cấu trúc if/else dùng để kiểm tra điều kiện và đưa ra kết quả khác nhau tùy vào giá trị của biến, giống như chọn một hướng đi phù hợp khi gặp nhiều lựa chọn.
---
...
>   - Thân hàm (body): Phần code nằm trong { }, gồm các câu lệnh if / else để kiểm tra giá trị avg.
...
Giúp tái sử dụng code nhiều lần mà không cần viết lại.
Code ngắn gọn và dễ đọc hơn.
Dễ sửa lỗi và bảo trì (chỉ cần sửa trong hàm).
Giúp chia chương trình thành các phần logic rõ ràng.
---
...
Cách hoạt động của if / else và cách hàm return trả về giá trị.
> - Em đã làm gì để vượt qua (thử nhiều lần, hỏi bạn, xem lại ví dụ, tra Google, …)?
Thử chạy code nhiều lần trong Console, xem lại ví dụ trong bài, và tra thêm tài liệu trên Google để hiểu cách hoạt động của hàm.
...
> Câu hỏi:
> - `document.getElementById("status")` đang làm gì?→ Lệnh này dùng để tìm phần tử HTML có id là status trong trang web và gán nó vào biến statusEl để JavaScript có thể thao tác với phần tử đó.
> - Sự kiện `"click"` xảy ra khi nào?→ Sự kiện "click" xảy ra khi người dùng nhấn chuột vào một phần tử, ví dụ như một nút button.
> - Trong đoạn code trên, khi nhấn nút `btnHello`, điều gì thay đổi trên trang?
→ Khi nhấn nút Chào, nội dung của thẻ <p id="status"> sẽ được thay đổi thành:
...
"input": xảy ra mỗi khi nội dung trong ô input thay đổi (khi gõ, xoá, dán chữ).
...
>   → DOM là mô hình biểu diễn cấu trúc của trang HTML dưới dạng các đối tượng, cho phép JavaScript truy cập, thay đổi nội dung, thuộc tính và kiểu dáng của các phần tử trong trang web................
> - Nêu 1 ví dụ “thao tác DOM” trong bài (ghi lại 1 dòng lệnh cụ thể).
statusEl.textContent = "Xin chào! Đây là nội dung được thay đổi bằng JavaScript.";

Lệnh này thay đổi nội dung của phần tử HTML.
---
...
→ Cả hai đoạn code đều bắt sự kiện click của nút btnHello và hiển thị thông báo alert khi người dùng nhấn nút.
> - Điểm khác nhau về cú pháp là gì (`document.getElementById` vs `$("#id")`, `addEventListener` vs `.on`)?
JavaScript thuần:
dùng document.getElementById("btnHello") để lấy phần tử và addEventListener để bắt sự kiện.
jQuery:
dùng $("#btnHello") để chọn phần tử và .on("click") để xử lý sự kiện, cú pháp ngắn gọn hơn.
> - Em hãy tra cứu nhanh “What is jQuery used for?” và ghi 2 ý chính:
>   1. Đơn giản hóa việc thao tác DOM và xử lý sự kiện trong JavaScript
>   2. Giúp viết code JavaScript ngắn gọn và dễ sử dụng hơn, đặc biệt cho hiệu ứng và AJAX
---
...
→ Em muốn làm thêm nút đổi theme (dark/light mode) cho trang web.
